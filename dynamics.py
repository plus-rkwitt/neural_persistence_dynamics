...
def run_epoch(
    ...
    for batch in dl:
        ...
        out, evd_obs, evd_msk, aux_obs = modules['recog_net'](batch,
                                                              args.device)

        # (2) draw z(t_0) from initial state
        # distribution Normal(z|mu(x),sigma(x))
        ...
        # (5) compute KL divergence between approximate
		# posterior q(z|x) and prior p(z)
        pz0_mean = pz0_logvar = torch.zeros(z0.size()).to(args.device)
        kld = normal_kl(qz0_mean, qz0_logvar, pz0_mean, pz0_logvar).sum(
            -1).unsqueeze(0)
...
def create_recog_backbone(args):
    if args.backbone == 'topdyn_only':
        return TDABackbone(args)
    elif args.backbone == 'ptsdyn_only':
        return PointNetBackbone(args)
    elif args.backbone == 'joint':
        return JointBackbone(args)
    else:
...
def main():
    ...
    if args.tps_frac > 0:
        ...
        indices = create_sampling_indices(len(ds), args.num_timepts,
                                          int(args.tps_frac * args.num_timepts))
        ds.subsample(indices)
    spinner.succeed()

    # make sure splits are the same for a given seed
    split_generator = torch.Generator().manual_seed(args.seed)
    trn_set, tst_set = torch.utils.data.random_split(ds, [0.8, 0.2],
    ...
    modules = nn.ModuleDict(
        {
            ...
            "ptogd_net": PathToGaussianDecoder(nn.Identity(),
                                               initial_sigma=1.0),
            "processor": processor,
    ...
    for epoch_cnt in range(args.n_epochs):
        ...
        for scorefn_key, trackers_key in product(scorefns, trackers):
            ...
            for aux_d in range(args.num_aux_dim):
                ...
                if writer:
                    writer.add_scalar(
                        "{}_{}/{}".format(scorefn_key, aux_d, trackers_key),
                        tmp, epoch_cnt)

        print(
            '{:04d} | trn_loss={:.4f} | trn_aux={:0.4f} | trn_kld={:0.4f} | trn_log_pxz={:0.4f} | avg_trn_mse={:0.4f} | avg_tst_mse={:0.4f} | avg_tst_r2s={:0.4f} | lr={:0.6f}'.format(
                epoch_cnt,
        ...
        if writer:
            writer.add_scalar("r2s_avg/trn", np.mean(scores['r2s_trn']),
                              epoch_cnt)
            writer.add_scalar("r2s_avg/tst", np.mean(scores['r2s_tst']),
                              epoch_cnt)
        ...
        for aux_d in range(args.num_aux_dim):
            if writer:
                ...
                writer.add_figure('r2s/tst_scatter_{}'.format(aux_d), plt.gcf(),
                                  epoch_cnt)
                plt.close()
